...
class RecognitionWriterPlugin(WorldPlugin):
  ...
  def __init__(self, check_function, approach_distance = 1):
    super().__init__(nodeName='recognition_writer')
    ...
    self.must_remove_srv = self.create_service(Empty, "/fbot_world/trigger_remove", lambda req: self.must_remove_trigger.set())
  ...
  def mustUpdate(self, description):
  ...
  def toLink(self, image_header, description, link='map'):
    ...
    try:
      ...
      self.get_logger().error('Transform does not exist.')
      return None
  ...
  def saveDescription(self, description, image_rgb):
  ...
  def removeUnavailableDescriptions(self, image_header, descriptions, distance_threshold=0.1):
  ...
  def onRecognition(self, recognition):
    ...
    for description in recognition.descriptions:
      self.get_logger().info("Description: " + str(description))
  ...
  def declareParameters(self):
    self.declare_parameter('topic', '/fbot_vision/fr/face_recognition3d')
...
def main(args=None) -> None:
  ...
  node = RecognitionWriterPlugin('face_recognition_writer')
